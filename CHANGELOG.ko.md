...
### 수정됨
- 🐛 **Browser Pilot v1.9.2**: Promise.race 타임아웃 클리어 누락 수정
  - `clearTimeout()` 추가로 rimraf 타임아웃 처리 시 메모리 누수 방지
  - rimraf가 타임아웃 전에 완료될 때 타이머를 즉시 정리
  - 이벤트 루프에 불필요한 타이머가 남지 않도록 개선
...
### 추가됨
- ✨ **Browser Pilot v1.8.0**: 트러블슈팅을 위한 재설치 명령어
  - **새 명령어**: `reinstall` - Browser Pilot 스크립트 완전 재설치 강제 실행
    - 실행 중인 데몬 중지
    - `.browser-pilot` 디렉토리 완전 삭제
    - 다음 명령 실행 시 SessionStart 훅을 통해 자동 재설치 트리거
  - **사용 사례**: 손상된 설치, 스크립트 미업데이트, 지속적인 문제 해결
  - **옵션**:
    - `-y, --yes`: 확인 프롬프트 건너뛰기
    - `-q, --quiet`: 출력 메시지 숨기기
  - **문서**: `commands-reference.md`에 포괄적인 가이드 추가
...
### 추가됨
- ✨ **Browser Pilot v1.7.3**: 마지막 방문 URL 자동 복원
  - **자동 복원 기능**: 데몬 재시작 시 마지막 방문 URL 자동 복원
    - 네비게이션, 뒤로가기, 앞으로가기, 새로고침, 맵 재생성 시 URL을 `.browser-pilot/last-url.txt`에 저장
    - 데몬 시작 시 마지막 URL을 로드하고 자동 이동 (`autoRestore: true`인 경우)
    - 프로젝트 설정에 새로운 옵션 `autoRestore` (기본값: `true`) 추가
  - **향상된 URL 지속성**: 모든 네비게이션 작업에서 URL 저장
    - Navigate 명령어
    - Back/Forward 명령어
    - Reload 명령어
    - 맵 재생성 (regen-map)
  - 원활한 워크플로우: 데몬 중지 후 네비게이션 외 명령 실행 시 자동으로 마지막 페이지로 복귀
...
### 변경됨
- 🔧 **Browser Pilot v1.7.2**: 코드 품질 개선
  - `helpers.ts`에 `logActionError` 헬퍼 함수 추가로 중앙 집중식 에러 로깅 구현
  - `emulation.ts`의 중복 에러 로깅 제거를 위한 리팩토링
  - `capture.ts`에 안전한 뷰포트 응답 검증을 위한 타입 가드 함수 추가
  - `as` 타입 단언을 적절한 타입 가드로 대체하여 타입 안전성 향상
...
### 추가됨
- ✨ **Browser Pilot v1.7.1**: 뷰포트 정보 조회 명령어
  - **새 명령어**: 뷰포트 및 화면 정보 조회 기능 추가
    - `get-viewport`: 현재 뷰포트 크기 조회 (너비, 높이, 디바이스 픽셀 비율)
    - `get-screen-info`: 포괄적인 화면 정보 조회 (화면 크기, 사용 가능 영역, 뷰포트, 스케일)
  - 액션 → 핸들러 → CLI 레이어 전체 통합

### 수정됨
- 🐛 **Browser Pilot v1.7.1**: 맵 생성 버그 수정
  - `generate-interaction-map.ts`의 XPath 이스케이프 구문 오류 수정
  - `map-manager.ts`의 에러 로깅 개선으로 디버깅 향상
  - 복잡한 페이지에서 발생하던 "Failed to extract interactive elements" 오류 해결
