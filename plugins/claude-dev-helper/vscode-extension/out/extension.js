...
class GitContentProvider {
    async provideTextDocumentContent(uri) {
        try {
            ...
            const { stdout } = await execAsync(`git show HEAD:"${relativePath}"`, { cwd: workspaceFolder.uri.fsPath });
            return stdout;
        }
        catch (error) {
            return '';
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.workspace.registerTextDocumentContentProvider('git-head', gitContentProvider));
    ...
    codeLensProvider = new code_lens_provider_1.GitDiffCodeLensProvider(() => true);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudeDevHelper.showDiff', async (uriOrString) => {
        const uri = typeof uriOrString === 'string' ? vscode.Uri.parse(uriOrString) : uriOrString;
        await showGitDiff(uri);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('claudeDevHelper.showBrowserDiff', async (uriOrString) => {
        const uri = typeof uriOrString === 'string' ? vscode.Uri.parse(uriOrString) : uriOrString;
        await showBrowserDiff(uri);
    }));
    // Command to enable inline diff mode
    context.subscriptions.push(vscode.commands.registerCommand('claudeDevHelper.enableInlineDiffMode', async () => {
        const config = vscode.workspace.getConfiguration('diffEditor');
...
async function showGitDiff(uri) {
    try {
        const workspaceFolder = vscode.workspace.getWorkspaceFolder(uri);
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('No workspace folder found');
        ...
        try {
            await execAsync(`git show HEAD:"${relativePath}"`, { cwd: workspaceFolder.uri.fsPath });
            ...
            await vscode.commands.executeCommand('vscode.diff', gitHeadUri, uri, `${fileName} (Working Tree ↔ HEAD)`, { preview: false });
            ...
            if (error.message?.includes('does not exist') || error.message?.includes('Path')) {
                vscode.window.showInformationMessage('This is a new file (not in HEAD). No diff available.');
            }
            else {
                throw error;
            }
...
async function showBrowserDiff(uri) {
    try {
        // Check if server is running
        const isRunning = await checkServer();
        if (!isRunning) {
            // Ask user if they want to start the server
            const choice = await vscode.window.showInformationMessage('Diff editor server is not running. Start it now?', 'Yes', 'No');
            if (choice !== 'Yes') {
                return;
            }
            // Start server
