...
def calculate_similarity(name1: str, name2: str) -> float:
    ...
    # 3. 접미사 매칭 보너스 (l, r 등)
    if (norm1.endswith('_l') and norm2.endswith('_l')) or \
       (norm1.endswith('_r') and norm2.endswith('_r')) or \
       (norm1.endswith('.l') and norm2.endswith('.l')) or \
       (norm1.endswith('.r') and norm2.endswith('.r')):
        bonus += 0.1
