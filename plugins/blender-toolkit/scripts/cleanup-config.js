...
async function stopDaemon(projectRoot) {
  const pidPath = path.join(projectRoot, '.blender-toolkit', 'daemon.pid');
  const shutdownFlagPath = path.join(projectRoot, '.blender-toolkit', 'daemon-to-stop.pid');

  // Check if PID file exists
  if (!fs.existsSync(pidPath)) {
    logger.log('Blender Toolkit: No daemon PID file found');
    return;
  }
  ...
  try {
    ...
    if (!processUtils.isValidPid(pid)) {
      logger.log('Blender Toolkit: Removed invalid daemon PID file (PID: ' + pidStr + ')');
  ...
  if (!processUtils.isProcessRunning(pid)) {
    logger.log('Blender Toolkit: Daemon process not running (PID: ' + pid + ')');
    processUtils.removeFileWithFallback(pidPath, logger);
    return;
  }

  logger.log('Blender Toolkit: Stopping daemon (PID: ' + pid + ')...');
  ...
  try {
    ...
    if (shutdownSuccess) {
      logger.log('✓ Blender Toolkit Daemon stopped gracefully via IPC (PID: ' + pid + ')');
      processUtils.removeFileWithFallback(pidPath, logger);
  ...
  if (result.success) {
    if (result.graceful) {
      logger.log('✓ Blender Toolkit Daemon stopped (forced graceful) (PID: ' + pid + ')');
    } else {
      logger.log('✓ Blender Toolkit Daemon stopped (forced kill) (PID: ' + pid + ')');
    }
...
function getSocketPath(projectRoot) {
  if (process.platform === 'win32') {
    ...
    // Unix: Domain socket
    return path.join(projectRoot, '.blender-toolkit', `${SOCKET_PATH_PREFIX}.sock`);
  }
}
...
function cleanupCacheFiles(projectRoot) {
  ...
  logger.log('Note: Local scripts preserved in .blender-toolkit/bt.js');
}
...
async function cleanupProject(hookInput) {
  try {
    ...
    logger.log('Blender Toolkit: Removing project from shared config...');
