...
function registerNavigationCommands(program) {
    ...
    program
        ...
        try {
            const response = await (0, daemon_helper_1.executeViaDaemon)('navigate', { url: options.url });
            if (response.success) {
    ...
    program
        ...
        .action(async () => {
        try {
            const response = await (0, daemon_helper_1.executeViaDaemon)('forward', {});
            if (response.success) {
                const data = response.data;
                if (data.success) {
    ...
    program
        ...
        .action(async (options) => {
        try {
            const response = await (0, daemon_helper_1.executeViaDaemon)('reload', { hard: options.hard });
            if (response.success) {
                const data = response.data;
